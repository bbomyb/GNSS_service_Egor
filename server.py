...
def download_brdc_from_bkg(obs_date: date, out_dir: str) -> str:
    year = obs_date.year
    doy = obs_date.strftime('%j')
    yy = obs_date.strftime('%y')

    url = f"https://igs.bkg.bund.de/root_ftp/IGS/BRDC/{year}/{doy}/BRDC00IGS_R_{year}{doy}0000_01D_MN.rnx.gz"

    out_gz = os.path.join(out_dir, f"brdc{doy}0.{yy}n.gz")
    out_rnx = os.path.join(out_dir, f"brdc{doy}0.{yy}n")
...
def run_rtklib_rel(rover_file: str, base_file: str, nav_file: str, out_file: str):
    """Запускает rnx2rtkp в режиме относительного позиционирования (RTK)."""
...
def handle_client(conn):
    ...
    try:
        ...
        # Извлекаем дату (можно и из базы - они должны совпадать)
        obs_date = extract_date_from_rinex(rover_path)

        # Скачиваем эфемериды
        eph_path = download_brdc_from_bkg(obs_date, out_dir=os.path.dirname(rover_path))

        # Запускаем RTKLIB в режиме относительного позиционирования
        result_path = rover_path.replace('.25o', '.pos')
        run_rtklib_rel(rover_path, base_path, eph_path, result_path)

        # Извлекаем последнюю строку решения
        last_solution_line = ""
